...
function computeCharacterGroupPositions() {
	characters.forEach(function(character){
		var sum = character.appearances.reduce(function(total, appearance){
			return total + groups.indexOf(appearance.scene.group);
		}, 0);
		character.averageScenePosition = sum/character.appearances.length;
	});
...
function sortGroupAppearances() {
	groups.forEach(function(group){
		group.appearances.sort(function(a,b){
			...
			diff = a.group.order-b.group.order; // NOTE: Always zero
			if (diff !== 0) {
				return diff;
			}

			// Average scene position.
			diff = a.averageScenePosition - b.averageScenePosition; // NOTE: Always zero
			if (diff !== 0) {
				return diff;
			}
...
// 
// Takes the list of appearances and spaces them evenly in a specific order.
// 
...
// 
// The scene's position is based on the average positions of the apperances in the group.
// 
function computeScenePositions() {
	...
	scenes.forEach(function(scene) {
		...
		if (orientation === 'vertical') {
			...
			scene.y = scene._y || Math.min(size[1], scale * scene.start + labelSize[1]);
		} else {
...
function createIntroductionNodes() {
	...
	// Get a array of appearances in each characters first scene.
...
function computeIntroductionPositions() {
	...
	function resolveCollisionsHorizontal(introduction, i){
		...
		while (true) {
			...
			amount = collisionBounds[1][1] - introBounds[0][1];
	...
	// Returns [[x0, y0], [x1, y1]] the coordinates for the top left, and bottom right.
	function bBox(arr) {
		var x0,x1,y0,y1;
